...
```{r plotcount-zh, dependson = "tidy_hongloumeng", fig.width=6, fig.height=5, fig.cap="《红楼梦》中最常见的词"}
library(showtext)
...
看起来“人”在所有作品里都属于最常见的词。关注一下意思相当于“说”的词汇：《红楼梦》中，“道”和“說”都非常多（有部分“道”可能是道士的道，因此比例只会更接近1）；《三言》里就只有“道”排在前面，“說”只有其一半左右；《子不语》两部中只有“曰”上榜。这可能是由于文体（文言多用曰）及时代（说对道的替代需要一个过程）的差异。
...
```{r plotcompare, dependson = "frequency", fig.width=10, fig.height=5.5, fig.cap="冯梦龙、袁枚与曹雪芹的作品词频比较"}
library(scales)
...
cor.test(data = frequency[frequency$author == "冯梦龙",], ~ proportion + `Cao`)
cor.test(data = frequency[frequency$author == "袁枚",], ~ proportion + `Cao`)
```

与我们在图中所见一致，词频在曹雪芹和冯梦龙的小说中相关度要高于曹雪芹和袁枚。基本上，这个结论也符合主观的观感。
