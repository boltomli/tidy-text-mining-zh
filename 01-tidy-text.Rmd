...
中文暂时缺少已整理好的类似 janeaustenr 这样的数据集，我们可以从《红楼梦》的文本开始。
...
```{r plotcount-zh, dependson = "tidy_hongloumeng", fig.width=6, fig.height=5, fig.cap="《红楼梦》常见词"}
library(showtext)
...
英文原著用威尔斯（19世纪末二十世纪初）的科幻小说和勃朗特姐妹（十九世纪，活动时间晚于奥斯汀）作品集分别与简·奥斯汀的作品（主要出版于十九世纪一〇年代）进行比较。中文我们可以使用冯梦龙的《三言》和袁枚《子不语》《续子不语》分别与《红楼梦》进行比较。冯梦龙[明]时间较早，而曹雪芹[清]与袁枚[清]处于同一时代，但题材上《红楼梦》与《三言》更为接近。这些作品间的相似程度将如何呢？
...
看起来“人”在所有作品里都属于最常见的词。关注一下意思相当于“说”的词汇：《红楼梦》中，“道”和“说”都非常多（当然，有部分“道”是道士的道）；《三言》里就只有“道”排在前面；《子不语》两部中只有“曰”上榜。这可能是由于文体（文言多用曰）及时代（说这个用法较后起）的差异。

现在，计算一下每个词在每位作家的作品中出现的频率，把数据框绑定到一起。我们可以使用 tidyr 中的 `spread` 和 `gather` 重新定形数据框，只留下我们画图需要的点，以便对三个作品集进行比较。
...
注释掉的行用 `str_extract()` 是因为来自古登堡计划的UTF-8编码文本为了表示强调（比如斜体）在词前后加了下划线，符号化的时候这些词不应该独立计数。在选用 `str_extract()` 之前做的初步数据探索中，诸如“\_any\_”就没有算成“any”。对于这里的中文数据集，由于分词方法不同，这一步骤无需使用。

现在绘图（图 \@ref(fig:plotcompare)）。
...
靠近斜线的点代表的词在相应的两组文本中有着相似的出现频率，比如“道”“是”“不”同时高频出现在曹雪芹和冯梦龙的文本中，而“不”“見”“上”同时高频出现在曹雪芹和袁枚的文本中。远离斜线的词在一个文本中比另一个要常见得多。比如，在曹雪芹-冯梦龙一侧的图中，诸如“寳玉”“太太”“賈”这些词（全是人名和称谓）在曹雪芹文本中多见，而在冯梦龙文本中就不多，同时“員外”“郎”在冯梦龙文本中更多见而非曹雪芹文本。比较袁枚与曹雪芹，袁枚用词“某”“曰”较多而曹雪芹不用，曹雪芹用词“這”“呢”较多而袁枚不用。

整体来看，注意到在图 \@ref(fig:plotcompare) 中曹雪芹-冯梦龙侧比曹雪芹-袁枚侧更接近过零点的斜线。同时注意曹雪芹-冯梦龙侧的词向更低频扩展更多；曹雪芹-袁枚侧在低频词处有片空白区域。这些特征说明曹雪芹与冯梦龙用词比曹雪芹与袁枚用词更为接近。还可以看到并非所有词在全部三个文本集中都能找到，曹雪芹-袁枚侧的数据点偏少。

我们用相关度检验量化一下这些词频集的相似和不同的程度。曹雪芹与冯梦龙词频的相关度如何？曹雪芹与袁枚呢？
...
正如我们在图中所见，词频在曹雪芹和冯梦龙的小说中相关度要高于曹雪芹和袁枚。基本上，这个结论也符合主观的观感。
