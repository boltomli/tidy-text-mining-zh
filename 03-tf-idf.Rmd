...
cutter <- worker(bylines = TRUE)
stem$text <- sapply(segment(stem$text, cutter), function(x){paste(x, collapse = " ")})
stem_words <- stem %>%
  unnest_tokens(word, text) %>%
...
这个结果已经足够有趣。但有些词是数字，具体数字在这里也许不是重点，我们可以尝试移除掉。
...
从图 \@ref(fig:stemclean) 中可以得出一些结论。如沈括主要描绘了个人所做观察，而《天工開物》包括了工匠操作的具体步骤；算术方面，劉徽似乎更关注楼台高度、水土面积的计算等，而《孫子算經》则有更多度量粮食的问题。
