...
注意，这将模型变成了每行术语每术语一个主题的格式。对于每个组合，模型计算了该术语来自该主题的概率。比如，术语“`r chapter_topics$term[2]`”在主题`r chapter_topics$topic[2]`中的概率只有`r chapter_topics$beta[2]`，但在其它几个主题里都达到 1% 以上。
...
每个值都是来自该文档的词属于该主题的估测比例。例如模型估测`r chapters_gamma$document[1]`文档中的每个词有`r chapters_gamma$gamma[1]`概率来自主题`r chapters_gamma$topic[1]`。
