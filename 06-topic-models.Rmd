...
然后即可使用 `LDA()` 函数创建一个四主题模型。在这个例子里我们知道要寻找4个主题，因为有4本书；在其它问题里可以尝试几个不同的 `k` 值。
...
注意，这将模型变成了每行术语每术语一个主题的格式。对于每个组合，模型计算了该术语来自该主题的概率。比如，术语“`r chapter_topics$term[2]`”在主题`r chapter_topics$topic[2]`中的概率只有`r percent(chapter_topics$beta[2])`，但在其它几个主题里都达到 1% 以上。
...
这些主题很清晰地与四本书联系了起来！“賈”无疑属于《紅樓夢》，“行者”应该属于《西遊記》，“兵”“將”属于《三國志演義》，而《水滸傳》没有很明显的代表词。我们还可注意到，既然 LDA 属于“模糊聚类”方法，有些词为多个主题共有，如“道”和“人”各属三个不同主题。调整停止词列表，此结果当有所不同。
...
每个值都是来自该文档的词属于该主题的估测比例。例如模型估测`r chapters_gamma$document[1]`文档中的每个词有`r percent(chapters_gamma$gamma[1])`概率来自主题`r chapters_gamma$topic[1]`。

现在有了主题概率，可以看看我们的无监督学习区分4本书的表现如何。我们预期一本书中的每章基本（或全部）出现在生成的对应主题中。
...
注意，看起来几乎每本书的全部章都被识别为唯一的单个主题。有没有哪个章节被识别到另一本书？首先我们用 `top_n()` 找到每章被关联最多的主题，即该章的“分类”。
...
我们可以看到，有不少分配给《三國志演義》的词实际上来自《西遊記》《水滸傳》或《紅樓夢》。比如“關 ”，《三國志演義》的主人公之一關羽的姓，同时也是《水滸傳》中比较活跃（可以通过查看词频确认）的關胜的姓，据书中所言，關羽乃關胜先祖。

另外，有可能有的词在一本书中从未出现过但仍然被分配给该书。LDA 算法有一定随意性，有可能用一个主题覆盖到多本书。
